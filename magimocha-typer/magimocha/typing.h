...
struct type_table {
    virtual std::unique_ptr<type_value_ref>
    find(std::shared_ptr<ast::var_type_data> t) = 0;
    virtual ~type_table() {}
};
class type_table_impl : public type_table {
    using tmap = std::unordered_map<std::shared_ptr<ast::var_type_data>,
    ...
    struct type_value_ref_impl : public type_value_ref {
        ...
        explicit type_value_ref_impl(tmap &map, tmap::const_iterator itr,
                                     std::shared_ptr<ast::var_type_data> t)
            : map(map), itr(itr), t(t) {}
        std::shared_ptr<ast::type_data> get() override { return itr->second; }
        void set(std::shared_ptr<ast::type_data> d) override {
            map.insert_or_assign(t, d);
        }
        operator bool() const { return itr != map.cend(); }
    };

  public:
    std::unique_ptr<type_value_ref>
    find(std::shared_ptr<ast::var_type_data> t) override {
...
struct type_schema_table {
    virtual std::optional<type_schema>
    get(std::shared_ptr<ast::typed_data> k) = 0;
...
struct type_schema_table_impl : type_schema_table {
    ...
    std::optional<type_schema>
    get(std::shared_ptr<ast::typed_data> k) override {
        auto itr = map.find(k);
        if(itr == map.end()) {
            return std::nullopt;
        }
...
struct make_scope_2_variable_table_table {
    ...
    get(std::shared_ptr<ast::make_scope> k) = 0;
};
struct make_scope_2_variable_table_table_impl
    : make_scope_2_variable_table_table {
    std::unordered_map<std::shared_ptr<ast::make_scope>,
                       std::shared_ptr<name::variable_table>>
        map;
    void set(std::shared_ptr<ast::make_scope> k,
             std::shared_ptr<name::variable_table> v) override {
        map.insert_or_assign(k, v);
    }
    std::shared_ptr<name::variable_table>
    get(std::shared_ptr<ast::make_scope> k) override {
        auto itr = map.find(k);
        if(itr == cend(map)) {
            return std::shared_ptr<name::variable_table>();
        }
        return itr->second;
    }
};
...
inline static type_schema
create_type_schema_from(std::shared_ptr<type_table> types, type_schema ts) {
    std::unordered_map<std::shared_ptr<ast::type_data>,
                       std::shared_ptr<ast::type_data>>
        map;
...
// void unify(std::shared_ptr<type_table> table, std::shared_ptr<ast::type_data>
// t1, std::shared_ptr<ast::type_data> t2);
std::shared_ptr<ast::type_data>
...
static inline std::shared_ptr<ast::type_data>
resolve_type(std::shared_ptr<type_table> table,
             std::shared_ptr<ast::type_data> t) {
...
// std::shared_ptr<ast::type_data> infer(std::shared_ptr<type_table>
// table,std::shared_ptr<ast::typed_data> expr);
...
void infer_all(context con, std::shared_ptr<ast::module_member> dm);
} // namespace tig::magimocha::typing