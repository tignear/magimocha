...
{
	auto vis = operation_to_function_applying_visitor{op_infos};
	return ast::visit<typename operation_to_function_applying_visitor::R>(vis, td);
}
...
struct extract_operator_info_visitor
{
	...
	std::unordered_map<std::shared_ptr<ast::ast_leaf>, std::shared_ptr<operator_info_table>> &map;
	extract_operator_info_visitor(
		std::shared_ptr<operator_info_table> p,
		std::unordered_map<std::shared_ptr<ast::ast_leaf>, std::shared_ptr<operator_info_table>> &map) : p(p), map(map)
	{
	}
	void operator()(ast::op_token_function_applying es)
	{
		for (auto &&e : es.args())
		{
			extract_operator_info(p, ast::to_ast_leaf(e), map);
		}
	}
	template <class T>
	void operator()(T)
	...
	template <class T>
	void operator()(std::shared_ptr<T> e)
	{
		extract_operator_info(p, ast::to_ast_leaf(e), map);
	}
};
	...
	std::unordered_map<std::shared_ptr<ast::ast_leaf>, std::shared_ptr<operator_info_table>> &map)
{
	struct visitor
	{
		...
		auto operator()(std::shared_ptr<ast::declaration_function> x)
		{
			...
			extract_operator_info(np, ast::to_ast_leaf(x->body()), map);
			return r;
		}
		auto operator()(std::shared_ptr<ast::signed_number_literal> x)
		{
		...
		auto operator()(std::shared_ptr<ast::apply_function> x)
		{
			extract_operator_info(p, ast::to_ast_leaf(x->target()), map);
			for (auto &&e : x->args())
			{
				extract_operator_info(p, ast::to_ast_leaf(e), map);
			}
		...
		auto operator()(std::shared_ptr<ast::expression_block> x)
		{
			...
			for (auto e : x->value())
			{
				extract_operator_info(np, ast::to_ast_leaf(e), map);
			}
			return r;
		}
		auto operator()(std::shared_ptr<ast::operation> x)
		...
		auto operator()(std::shared_ptr<ast::named_function> x)
		{
			extract_operator_info(p, ast::to_ast_leaf(x->body()), map);
