...
template <class R, class Visitor>
static R visit(Visitor &v, std::shared_ptr<make_scope> node) {
    switch(node->type()) {
    case leaf_type::declaration_function:
        return v(std::static_pointer_cast<declaration_function>(node));
    case leaf_type::named_function:
...
template <class R> struct to_ast_elem {
    std::shared_ptr<R> s;
    ...
    to_ast_elem(std::shared_ptr<ast::literal_> s) {
        ast::visit<std::monostate>(
            *this, std::static_pointer_cast<ast::expression>(s));
    }
    template <class T> to_ast_elem(std::shared_ptr<T> s) : s(s) {}
    std::shared_ptr<R> operator()() const { return s; }
    operator std::shared_ptr<R>() const { return s; }

    template <class U> std::monostate operator()(std::shared_ptr<U> u) {
        s = u;
